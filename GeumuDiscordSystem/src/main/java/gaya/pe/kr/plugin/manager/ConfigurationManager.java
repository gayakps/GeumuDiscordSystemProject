package gaya.pe.kr.plugin.manager;
...
public class ConfigurationManager {
    ...
    Plugin plugin = GeumuDiscordSystem.getPlugin();
    File pluginDataFolder = GeumuDiscordSystem.getPlugin().getDataFolder();
    ...
    private FileConfiguration getDefaultConfiguration(File path, String resourcePath) {
        try {
            ...
            GeumuDiscordSystem.log(String.format("&f[ &b%s &f]의&f 설정파일이 정상 &e생성&f 되었습니다", path.getName()));
    ...
    public void saveConfiguration(Plugin plugin, FileConfiguration configuration, String relativePath) {
        try {
            ...
            GeumuDiscordSystem.log(String.format("&f[&6%s&f]의 파일이 &e성공적&f으로 저장 되었습니다", relativePath));
        } catch (IOException e) {
            e.printStackTrace();
            GeumuDiscordSystem.log(String.format("&f[&6%s&f]의 파일의 저장을 &4실패 했습니다", relativePath));
        }
